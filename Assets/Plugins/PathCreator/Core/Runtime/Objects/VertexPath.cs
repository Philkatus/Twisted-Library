...
namespace PathCreation
{
    ...
    public class VertexPath
    {
        ...
        /// Calculate time data for closest point on the path from given world point that is not obscured by environment
        public Vector3 GetClosestNotConcealedPointOnPathData(Vector3 worldPoint)
        {
            Vector3 localPoint = MathUtility.InverseTransformPoint(worldPoint, transform, space);
            ...
            for (int i = 0; i < localPoints.Length; i++)
            {
                ...
                float closestSegmentLengthTemp = (localPoints[i] - localPoints[nextI]).magnitude;
                float temp = (closestPoint - localPoints[i]).magnitude / closestSegmentLengthTemp;
                TimeOnPathData dataTemp = new TimeOnPathData(i, nextI, temp);
                Vector3 localResultTemp = Vector3.Lerp(localPoints[dataTemp.previousIndex], localPoints[dataTemp.nextIndex], dataTemp.percentBetweenIndices);

                Vector3 snappingPoint = MathUtility.TransformPoint(localResultTemp, transform, space);
                LayerMask mask = LayerMask.GetMask("Environment");
                RaycastHit hit;
                if (sqrDst < minSqrDst && !Physics.Linecast(Camera.main.transform.position, snappingPoint, out hit, mask, QueryTriggerInteraction.Ignore))
                {
                    minSqrDst = sqrDst;
                    closestPoint = closestPointOnSegment;
                    closestSegmentIndexA = i;
            ...
            float closestSegmentLength = (localPoints[closestSegmentIndexA] - localPoints[closestSegmentIndexB]).magnitude;
            float t = (closestPoint - localPoints[closestSegmentIndexA]).magnitude / closestSegmentLength;
            TimeOnPathData data = new TimeOnPathData(closestSegmentIndexA, closestSegmentIndexB, t);
            Vector3 localResult = Vector3.Lerp(localPoints[data.previousIndex], localPoints[data.nextIndex], data.percentBetweenIndices);
